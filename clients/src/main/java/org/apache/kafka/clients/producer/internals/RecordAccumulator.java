...
public final class RecordAccumulator {
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            synchronized (dq) {
                ...
                RecordAppendResult appendResult = tryAppend(timestamp, key, headers, value, callback, dq);
                if (appendResult != null)
                    return appendResult;
            }

            // we don't have an in-progress record batch try to allocate a new batch
            int size = Math.max(this.batchSize, Records.LOG_OVERHEAD + Record.recordSize(key, headers, value));
            ...
            synchronized (dq) {
                ...
                FutureRecordMetadata future = Utils.notNull(batch.tryAppend(timestamp, key, headers, value, callback, time.milliseconds()));
     ...
     */
    private RecordAppendResult tryAppend(long timestamp, byte[] key, byte[] headers, byte[] value, Callback callback, Deque<RecordBatch> deque) {
        RecordBatch last = deque.peekLast();
        if (last != null) {
            FutureRecordMetadata future = last.tryAppend(timestamp, key, headers, value, callback, time.milliseconds());
            if (future == null)
                last.records.close();
            else
