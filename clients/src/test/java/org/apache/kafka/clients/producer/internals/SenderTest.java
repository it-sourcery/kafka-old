...
public class SenderTest {
    ...
    public void testSimple() throws Exception {
        long offset = 0;
        Future<RecordMetadata> future = accumulator.append(tp, 0L, "key".getBytes(), null, "value".getBytes(), null, MAX_BLOCK_TIMEOUT).future;
    ...
    public void testRetries() throws Exception {
        ...
        try {
            ...
            future = accumulator.append(tp, 0L, "key".getBytes(), null, "value".getBytes(), null, MAX_BLOCK_TIMEOUT).future;
            sender.run(time.milliseconds()); // send produce request
    ...
    public void testSendInOrder() throws Exception {
        ...
        try {
            ...
            accumulator.append(tp2, 0L, "key1".getBytes(), null, "value1".getBytes(), null, MAX_BLOCK_TIMEOUT);
            sender.run(time.milliseconds()); // connect
            sender.run(time.milliseconds()); // send produce request
            String id = client.requests().peek().request().destination();
            ...
            accumulator.append(tp2, 0L, "key2".getBytes(), null, "value2".getBytes(), null, MAX_BLOCK_TIMEOUT);
    ...
    public void testMetadataTopicExpiry() throws Exception {
        ...
        Future<RecordMetadata> future = accumulator.append(tp, time.milliseconds(), "key".getBytes(), null, "value".getBytes(), null, MAX_BLOCK_TIMEOUT).future;
        sender.run(time.milliseconds());
        ...
        future = accumulator.append(tp, time.milliseconds(), "key".getBytes(), null, "value".getBytes(), null, MAX_BLOCK_TIMEOUT).future;
        sender.run(time.milliseconds());
        assertTrue("Topic not added to metadata", metadata.containsTopic(tp.topic()));
        metadata.update(cluster, time.milliseconds());
        sender.run(time.milliseconds());  // send produce request
